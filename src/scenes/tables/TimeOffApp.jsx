...
const TimeoffApp = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get('https://abidiserver.vercel.app/api/timeoff');
        setProjects(data);
      } catch (error) {
  ...
  const handleApprove = async (id, reason, off_date, end_date, email, name) => {
    try {
      const res = await axios.post("https://abidiserver.vercel.app/api/timeoff/approve", { id });
  
      alert(res.data.message);
  
      await axios.get("https://abidiserver.vercel.app/api/timeoff/mail", {
        params: {
          email,
          reason,
          off_date,
